...
class Payment extends CActiveRecord {
    ...
    public function search() {
        ...
        $criteria->compare('service_pack.id', $this->service_pack_search, true);
    ...
    public function getTotals($ids) {
        ...
        $command = $connection->createCommand("
            SELECT SUM(price) AS total_price,valuta  
            FROM payments as p
            LEFT JOIN service_packs as s ON p.service_pack_id = s.id
            WHERE p.id IN ($ids)            
            GROUP BY valuta
            ORDER BY valuta
        ");
